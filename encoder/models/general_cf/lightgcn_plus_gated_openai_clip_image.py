...
from models.modules.gated_fusion import GatedFusion

init = nn.init.xavier_uniform_
uniformInit = nn.init.uniform

class LightGCN_plus_gated_openai_clip_image(BaseModel):
    def __init__(self, data_handler):
        super(LightGCN_plus_gated_openai_clip_image, self).__init__(data_handler)
        ...
        self.item_text_embeds = t.tensor(configs["itmprf_embeds"]).float().cuda()
    ...
    def cal_loss(self, batch_data):
        ...
        itmprf_embeds ,gate_values= self.fusion(
            text_item_embeds,
            image_item_embeds
        )
        self.gate_values = gate_values.detach()
        if not hasattr(self, '_gate_checked'):
            print("Gate shape:", gate_values.shape)
            print("Gate mean:", gate_values.mean().item())
            print("Gate min:", gate_values.min().item())
            print("Gate max:", gate_values.max().item())
